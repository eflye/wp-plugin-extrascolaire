...
Tarifs et statut « sans repas » sont **datés** (P1-16, schéma 4.17.0) : la carte d'un jour est résolue avec le statut de ce jour (`Psc_Sans_Repas`, table `psc_sans_repas`), et `psc_billing_services($day, $sans_repas, $regle, $date)` compare les tarifs en vigueur ce jour-là (`psc_billing_tariffs($date)`, table `psc_tarifs`). Une facture regroupe ses jours par prestation et par tarif. Tout montant se calcule en centimes entiers (`['centimes']` de la grille) et ne passe en euros qu'une fois, pour l'enregistrement et l'affichage.
