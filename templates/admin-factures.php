...
$psc_notices = array(
    ...
    'sent_all'    => array('updated', sprintf(__('Toutes les factures ont été envoyées (%d).', 'periscolaire-registration'), $psc_ok)),
    'sent_all_partial' => array('error', sprintf(__('%1$d facture(s) envoyée(s), %2$d échec(s). Les factures en échec restent « non envoyées » : relancez l’envoi pour elles seules.', 'periscolaire-registration'), $psc_ok, $psc_ko)),
...
<tr>
    ...
    <td>
        ...
        <?php else: ?>
            <span style="color:#666"><?php esc_html_e('Non envoyée', 'periscolaire-registration'); ?></span>
        <?php endif; ?>
        <?php $inv_bilan = $invoice_bilans[(int) $inv->id] ?? null; ?>
        <?php if ($inv_bilan && $inv_bilan[Psc_Envois::ECHEC] > 0): ?>
            <br><span style="color:#B32D2E" data-testid="facture-echec-<?php echo esc_attr($inv->id); ?>"><?php esc_html_e('Échec du dernier envoi', 'periscolaire-registration'); ?></span>
        <?php endif; ?>
