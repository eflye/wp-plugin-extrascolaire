...
<tr><td colspan="11"><?php esc_html_e('Aucun enfant enregistré', 'periscolaire-registration'); ?><?php echo $selected_year_id ? ' ' . esc_html__('pour cette année', 'periscolaire-registration') : ' ' . esc_html__(" (créez d'abord une année scolaire)", 'periscolaire-registration'); ?>.</td></tr>
...
<?php if ((int) $selected_year_id === (int) $psc_active_year_id): ?><a class="button button-small" href="<?php echo esc_url(admin_url('admin.php?page=psc_assurances&child_id=' . (int) $c->id)); ?>">Examiner</a><?php endif; ?>
...
<?php echo esc_html(Psc_Assurances::status_label(Psc_Assurances::status($c))); ?> — <?php esc_html_e('Fournie le', 'periscolaire-registration'); ?> <?php echo esc_html(date_i18n('d/m/Y', strtotime($c->assurance_uploaded_at))); ?>
</a>
<?php else: ?>
