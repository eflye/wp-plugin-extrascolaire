...
5. Téléchargez la nouvelle Release, puis vérifiez que l'archive n'a pas été altérée : placez `SHA256SUMS`, publié avec chaque Release, dans le même dossier que les zips, puis lancez `sha256sum -c SHA256SUMS` (ou comparez l'empreinte affichée par `shasum -a 256` sous macOS). Chaque version publiée a déjà passé l'ensemble des tests et un essai de mise à jour depuis la version précédente. Remplacez ensuite les fichiers du dossier du plugin dans `wp-content/plugins/periscolaire-registration/`. Ne supprimez pas la base ni le dossier privé pendant cette opération.
