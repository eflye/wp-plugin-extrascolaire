...
Une fois enregistrée par la mairie, la famille saisit son e-mail sur la page publique et reçoit un lien de connexion valable 30 minutes par défaut (réglable — voir [Réglages](#réglages)). Aucun mot de passe à créer ni à retenir. La session ouverte dure 12 heures (cookie signé, non modifiable côté client). Le lien reste utilisable pendant toute sa fenêtre de validité — pourquoi, voir [Sécurité](#sécurité) : les passerelles de sécurité des messageries pré-cliquent les liens des e-mails entrants, et un lien à usage unique était brûlé avant même que le parent ne l'ouvre.
